...
— … да, их сын, Гарри…
'''  # отрывок текста
lst_from_str = str_1.split()  # разбиение текста по словам в список
dct_word_count = {i: lst_from_str.count(i) for i in lst_from_str}  # формирование словаря кол-ва повтора слов в тексте

sorted_word_count_desc = sorted(dct_word_count.items(), key=lambda x: x[1], reverse=True)
"""сортировка по убыванию по самым часто повторящимся словам"""

dct_rating = {}
for k, v in enumerate(sorted_word_count_desc, start=1):
    dct_rating[k] = v
"""формирование словаря-рейтинга самых повторяющихся слов"""

dct_rating_counts = {}
for k, v in dct_rating.items():
    dct_rating_counts[tuple([k, v[1]])] = v[0]
print('''Первый словарь: ключ - (рейтинг,кол повторов слова в тексте):  значение - слово''')
print(dct_rating_counts)
print()

'''формирование первого словаря задания:  ключ - (рейтинг,кол повторов слова в тексте): значение - слово'''
dct_key_word = {}
for k, v in dct_rating_counts.items():
    dct_key_word[v] = k
print('''Второй словарь: ключ - слово: значение - (рейтинг,кол повторов слова в тексте''')
print(dct_key_word)
'''формирование второго словаря задания:  ключ - слово: значение - (рейтинг,кол повторов слова в тексте)'''
